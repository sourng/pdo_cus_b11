...
$lesson_grid = NULL; // Initialize page object first

class clesson_grid extends clesson {
	...
	// Page object name
	var $PageObjName = 'lesson_grid';

	// Grid form hidden field names
	var $FormName = 'flessongrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["lesson"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function SetRecordKey(&$key, $rs) {
		...
		$key .= $rs->fields('lesson_id');
		if ($key <> "") $key .= $GLOBALS["EW_COMPOSITE_KEY_SEPARATOR"];
		$key .= $rs->fields('subject_id');
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 2) {
			if (strval($arKeys[0]) <> "")
				$this->lesson_id->CurrentValue = strval($arKeys[0]); // lesson_id
			else
				$bValidKey = FALSE;
			if (strval($arKeys[1]) <> "")
				$this->subject_id->CurrentValue = strval($arKeys[1]); // subject_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetupMasterParms() {
		...
		if ($sMasterTblVar == "subject") {
			...
			if ($GLOBALS["subject"]->EventCancelled) $this->EventCancelled = TRUE;
		}
