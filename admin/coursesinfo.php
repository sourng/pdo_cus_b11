...
class ccourses extends cTable {
	...
	function __construct() {
		...
		// course_id
		$this->course_id = new cField('courses', 'courses', 'x_course_id', 'course_id', '`course_id`', '`course_id`', 3, -1, FALSE, '`course_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// course_title
		$this->course_title = new cField('courses', 'courses', 'x_course_title', 'course_title', '`course_title`', '`course_title`', 200, -1, FALSE, '`course_title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->course_title->Sortable = TRUE; // Allow sort
		$this->fields['course_title'] = &$this->course_title;

		// course_description
		$this->course_description = new cField('courses', 'courses', 'x_course_description', 'course_description', '`course_description`', '`course_description`', 200, -1, FALSE, '`course_description`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->course_description->Sortable = TRUE; // Allow sort
		$this->fields['course_description'] = &$this->course_description;

		// course_detail
		$this->course_detail = new cField('courses', 'courses', 'x_course_detail', 'course_detail', '`course_detail`', '`course_detail`', 201, -1, FALSE, '`course_detail`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->course_detail->Sortable = TRUE; // Allow sort
		$this->fields['course_detail'] = &$this->course_detail;

		// image
		$this->image = new cField('courses', 'courses', 'x_image', 'image', '`image`', '`image`', 200, -1, TRUE, '`image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->image->Sortable = TRUE; // Allow sort
		$this->fields['image'] = &$this->image;

		// status
		$this->status = new cField('courses', 'courses', 'x_status', 'status', '`status`', '`status`', 16, -1, FALSE, '`status`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'RADIO');
	...
	function Insert(&$rs) {
		...
		if ($bInsert) {
			...
			$this->course_id->setDbValue($conn->Insert_ID());
			$rs['course_id'] = $this->course_id->DbValue;
		}
		return $bInsert;
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["course_id"]))
				$arKeys[] = $_POST["course_id"];
			elseif (isset($_GET["course_id"]))
				$arKeys[] = $_GET["course_id"];
			else
	...
	function GetKeyFilter() {
		...
		foreach ($arKeys as $key) {
			...
			$this->course_id->CurrentValue = $key;
	...
	function LoadListRowValues(&$rs) {
		...
		$this->course_title->setDbValue($rs->fields('course_title'));
		$this->course_description->setDbValue($rs->fields('course_description'));
		$this->course_detail->setDbValue($rs->fields('course_detail'));
	...
	function RenderEditRow() {
		...
		$this->course_title->EditValue = $this->course_title->CurrentValue;
		...
		$this->course_description->EditValue = $this->course_description->CurrentValue;
		...
		$this->course_detail->EditValue = $this->course_detail->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->course_title->Exportable) $Doc->ExportCaption($this->course_title);
					if ($this->course_description->Exportable) $Doc->ExportCaption($this->course_description);
					if ($this->course_detail->Exportable) $Doc->ExportCaption($this->course_detail);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->course_title->Exportable) $Doc->ExportField($this->course_title);
						if ($this->course_description->Exportable) $Doc->ExportField($this->course_description);
						if ($this->course_detail->Exportable) $Doc->ExportField($this->course_detail);
