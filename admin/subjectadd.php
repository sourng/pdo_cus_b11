...
$subject_add = NULL; // Initialize page object first

class csubject_add extends csubject {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "subjectview.php")
	...
	function ValidateForm() {
		...
		if (in_array("lesson", $DetailTblVar) && $GLOBALS["lesson"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("lesson", $DetailTblVar) && $GLOBALS["lesson"]->DetailAdd) {
				$GLOBALS["lesson"]->subject_id->setSessionValue($this->subject_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["lesson_grid"])) $GLOBALS["lesson_grid"] = new clesson_grid(); // Get detail page object
				$AddRow = $GLOBALS["lesson_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["lesson"]->subject_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("lesson", $DetailTblVar)) {
				...
				if ($GLOBALS["lesson_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["lesson_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["lesson_grid"]->CurrentMode = "add";
					$GLOBALS["lesson_grid"]->CurrentAction = "gridadd";
...
// Create page object
if (!isset($subject_add)) $subject_add = new csubject_add();
...
var CurrentForm = fsubjectadd = new ew_Form("fsubjectadd", "add");

// Validate form
fsubjectadd.Validate = function() {
...
fsubjectadd.Form_CustomValidate = 
...
fsubjectadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $subject_add->ShowPageHeader(); ?>
<?php
$subject_add->ShowMessage();
?>
<form name="fsubjectadd" id="fsubjectadd" class="<?php echo $subject_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($subject_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $subject_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="subject">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($subject_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($subject->course_id->Visible) { // course_id ?>
	<div id="r_course_id" class="form-group">
		<label id="elh_subject_course_id" for="x_course_id" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->course_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->course_id->CellAttributes() ?>>
...
<?php if ($subject->instructor_id->Visible) { // instructor_id ?>
	<div id="r_instructor_id" class="form-group">
		<label id="elh_subject_instructor_id" for="x_instructor_id" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->instructor_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->instructor_id->CellAttributes() ?>>
...
<?php if ($subject->subject_title->Visible) { // subject_title ?>
	<div id="r_subject_title" class="form-group">
		<label id="elh_subject_subject_title" for="x_subject_title" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->subject_title->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->subject_title->CellAttributes() ?>>
...
<?php if ($subject->subject_description->Visible) { // subject_description ?>
	<div id="r_subject_description" class="form-group">
		<label id="elh_subject_subject_description" for="x_subject_description" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->subject_description->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->subject_description->CellAttributes() ?>>
...
<?php if ($subject->subject_detail->Visible) { // subject_detail ?>
	<div id="r_subject_detail" class="form-group">
		<label id="elh_subject_subject_detail" for="x_subject_detail" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->subject_detail->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->subject_detail->CellAttributes() ?>>
...
<?php if ($subject->image->Visible) { // image ?>
	<div id="r_image" class="form-group">
		<label id="elh_subject_image" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->image->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->image->CellAttributes() ?>>
...
<?php if ($subject->price->Visible) { // price ?>
	<div id="r_price" class="form-group">
		<label id="elh_subject_price" for="x_price" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->price->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->price->CellAttributes() ?>>
...
<?php if ($subject->dist->Visible) { // dist ?>
	<div id="r_dist" class="form-group">
		<label id="elh_subject_dist" for="x_dist" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->dist->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->dist->CellAttributes() ?>>
...
<?php if ($subject->unit->Visible) { // unit ?>
	<div id="r_unit" class="form-group">
		<label id="elh_subject_unit" for="x_unit" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->unit->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->unit->CellAttributes() ?>>
...
<?php if ($subject->stutus->Visible) { // stutus ?>
	<div id="r_stutus" class="form-group">
		<label id="elh_subject_stutus" for="x_stutus" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->stutus->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->stutus->CellAttributes() ?>>
...
<?php if ($subject->create_date->Visible) { // create_date ?>
	<div id="r_create_date" class="form-group">
		<label id="elh_subject_create_date" class="<?php echo $subject_add->LeftColumnClass ?>"><?php echo $subject->create_date->FldCaption() ?></label>
		<div class="<?php echo $subject_add->RightColumnClass ?>"><div<?php echo $subject->create_date->CellAttributes() ?>>
...
<script type="text/javascript">
ew_CreateEditor("fsubjectadd", "x_create_date", 0, 0, <?php echo ($subject->create_date->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	if (in_array("lesson", explode(",", $subject->getCurrentDetailTable())) && $lesson->DetailAdd) {
?>
...
<?php } ?>
<?php if (!$subject_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $subject_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $subject_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
