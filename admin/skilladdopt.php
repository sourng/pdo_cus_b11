...
$skill_addopt = NULL; // Initialize page object first

class cskill_addopt extends cskill {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow()) { // Add successful
					$row = array();
					$row["x_skill_id"] = $this->skill_id->DbValue;
					$row["x_skill_title"] = ew_ConvertToUtf8($this->skill_title->DbValue);
					$row["x_skill_detail"] = ew_ConvertToUtf8($this->skill_detail->DbValue);
					$row["x_skill_image"] = ew_ConvertToUtf8($this->skill_image->DbValue);
					$row["x_status"] = ew_ConvertToUtf8($this->status->DbValue);
					if (!EW_DEBUG_ENABLED && ob_get_length())
						ob_end_clean();
	...
	function LoadFormValues() {
		...
		if (!$this->skill_title->FldIsDetailKey) {
			$this->skill_title->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_skill_title")));
		}
		if (!$this->skill_detail->FldIsDetailKey) {
			$this->skill_detail->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_skill_detail")));
		}
		if (!$this->skill_image->FldIsDetailKey) {
			$this->skill_image->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_skill_image")));
		}
		if (!$this->status->FldIsDetailKey) {
			$this->status->setFormValue(ew_ConvertFromUtf8($objForm->GetValue("x_status")));
		}
	}
	...
	function RestoreFormValues() {
		global $objForm;
		$this->skill_title->CurrentValue = ew_ConvertToUtf8($this->skill_title->FormValue);
		$this->skill_detail->CurrentValue = ew_ConvertToUtf8($this->skill_detail->FormValue);
		$this->skill_image->CurrentValue = ew_ConvertToUtf8($this->skill_image->FormValue);
		$this->status->CurrentValue = ew_ConvertToUtf8($this->status->FormValue);
	}
...
// Create page object
if (!isset($skill_addopt)) $skill_addopt = new cskill_addopt();

// Page init
$skill_addopt->Page_Init();

// Page main
$skill_addopt->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$skill_addopt->Page_Render();
?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "addopt";
var CurrentForm = fskilladdopt = new ew_Form("fskilladdopt", "addopt");

// Validate form
fskilladdopt.Validate = function() {
...
fskilladdopt.Form_CustomValidate = 
...
fskilladdopt.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
$skill_addopt->ShowMessage();
?>
<form name="fskilladdopt" id="fskilladdopt" class="ewForm form-horizontal" action="skilladdopt.php" method="post">
<?php if ($skill_addopt->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $skill_addopt->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="skill">
<input type="hidden" name="a_addopt" id="a_addopt" value="A">
<?php if ($skill->skill_title->Visible) { // skill_title ?>
	<div class="form-group">
		<label class="col-sm-2 control-label ewLabel" for="x_skill_title"><?php echo $skill->skill_title->FldCaption() ?></label>
		<div class="col-sm-10">
...
<?php if ($skill->skill_detail->Visible) { // skill_detail ?>
	<div class="form-group">
		<label class="col-sm-2 control-label ewLabel" for="x_skill_detail"><?php echo $skill->skill_detail->FldCaption() ?></label>
		<div class="col-sm-10">
...
<?php if ($skill->skill_image->Visible) { // skill_image ?>
	<div class="form-group">
		<label class="col-sm-2 control-label ewLabel" for="x_skill_image"><?php echo $skill->skill_image->FldCaption() ?></label>
		<div class="col-sm-10">
...
<?php if ($skill->status->Visible) { // status ?>
	<div class="form-group">
		<label class="col-sm-2 control-label ewLabel" for="x_status"><?php echo $skill->status->FldCaption() ?></label>
		<div class="col-sm-10">
