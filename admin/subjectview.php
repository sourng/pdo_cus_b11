...
$subject_view = NULL; // Initialize page object first

class csubject_view extends csubject {
	...
	function __construct() {
		...
		if (@$_GET["course_id"] <> "") {
			$this->RecKey["course_id"] = $_GET["course_id"];
			$KeyUrl .= "&amp;course_id=" . urlencode($this->RecKey["course_id"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			...
			if (@$_GET["course_id"] <> "") {
				...
				$this->RecKey["course_id"] = $this->course_id->QueryStringValue;
			} elseif (@$_POST["course_id"] <> "") {
				$this->course_id->setFormValue($_POST["course_id"]);
				$this->RecKey["course_id"] = $this->course_id->FormValue;
			} else {
				$bLoadCurrentRecord = TRUE;
			}
			if (@$_GET["instructor_id"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("subjectlist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->subject_id->CurrentValue) == strval($this->Recordset->fields('subject_id')) && strval($this->course_id->CurrentValue) == strval($this->Recordset->fields('course_id')) && strval($this->instructor_id->CurrentValue) == strval($this->Recordset->fields('instructor_id'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "subjectlist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "subjectlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_lesson"
		$item = &$option->Add("detail_lesson");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("lesson", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["lesson_grid"] && $GLOBALS["lesson_grid"]->DetailView && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
			$links .= "<li><a class=\"ewRowLink ewDetailView\" data-action=\"view\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailViewLink")) . "\" href=\"" . ew_HtmlEncode($this->GetViewUrl(EW_TABLE_SHOW_DETAIL . "=lesson")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailViewLink")) . "</a></li>";
			if ($DetailViewTblVar <> "") $DetailViewTblVar .= ",";
			$DetailViewTblVar .= "lesson";
		}
		if ($GLOBALS["lesson_grid"] && $GLOBALS["lesson_grid"]->DetailEdit && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
			$links .= "<li><a class=\"ewRowLink ewDetailEdit\" data-action=\"edit\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailEditLink")) . "\" href=\"" . ew_HtmlEncode($this->GetEditUrl(EW_TABLE_SHOW_DETAIL . "=lesson")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailEditLink")) . "</a></li>";
			if ($DetailEditTblVar <> "") $DetailEditTblVar .= ",";
			$DetailEditTblVar .= "lesson";
		}
		if ($GLOBALS["lesson_grid"] && $GLOBALS["lesson_grid"]->DetailAdd && $Security->IsLoggedIn() && $Security->IsLoggedIn()) {
			$links .= "<li><a class=\"ewRowLink ewDetailCopy\" data-action=\"add\" data-caption=\"" . ew_HtmlTitle($Language->Phrase("MasterDetailCopyLink")) . "\" href=\"" . ew_HtmlEncode($this->GetCopyUrl(EW_TABLE_SHOW_DETAIL . "=lesson")) . "\">" . ew_HtmlImageAndText($Language->Phrase("MasterDetailCopyLink")) . "</a></li>";
	...
	function ExportData() {
		...
		// Export detail records (lesson)
		if (EW_EXPORT_DETAIL_RECORDS && in_array("lesson", explode(",", $this->getCurrentDetailTable()))) {
			global $lesson;
			if (!isset($lesson)) $lesson = new clesson;
			$rsdetail = $lesson->LoadRs($lesson->GetDetailFilter()); // Load detail records
			if ($rsdetail && !$rsdetail->EOF) {
				...
				if ($this->Export <> "csv" || EW_EXPORT_DETAIL_RECORDS_FOR_CSV) {
					...
					$oldtbl = $Doc->Table;
					$Doc->Table = $lesson;
					$lesson->ExportDocument($Doc, $rsdetail, 1, $detailcnt);
					$Doc->Table = $oldtbl;
				}
	...
	function SetupDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("lesson", $DetailTblVar)) {
				...
				if ($GLOBALS["lesson_grid"]->DetailView) {
					$GLOBALS["lesson_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($subject_view)) $subject_view = new csubject_view();

// Page init
$subject_view->Page_Init();

// Page main
$subject_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$subject_view->Page_Render();
?>
...
var CurrentForm = fsubjectview = new ew_Form("fsubjectview", "view");

// Form_CustomValidate event
fsubjectview.Form_CustomValidate = 
...
fsubjectview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php if ($subject->Export == "") { ?>
<div class="ewToolbar">
<?php $subject_view->ExportOptions->Render("body") ?>
<?php
	foreach ($subject_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php $subject_view->ShowPageHeader(); ?>
<?php
$subject_view->ShowMessage();
?>
<?php if (!$subject_view->IsModal) { ?>
<?php if ($subject->Export == "") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($subject_view->Pager)) $subject_view->Pager = new cPrevNextPager($subject_view->StartRec, $subject_view->DisplayRecs, $subject_view->TotalRecs, $subject_view->AutoHidePager) ?>
<?php if ($subject_view->Pager->RecordCount > 0 && $subject_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($subject_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $subject_view->PageUrl() ?>start=<?php echo $subject_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($subject_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $subject_view->PageUrl() ?>start=<?php echo $subject_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $subject_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($subject_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $subject_view->PageUrl() ?>start=<?php echo $subject_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($subject_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $subject_view->PageUrl() ?>start=<?php echo $subject_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $subject_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<?php } ?>
<form name="fsubjectview" id="fsubjectview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($subject_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $subject_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="subject">
<input type="hidden" name="modal" value="<?php echo intval($subject_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($subject->subject_id->Visible) { // subject_id ?>
	<tr id="r_subject_id">
		<td class="col-sm-2"><span id="elh_subject_subject_id"><?php echo $subject->subject_id->FldCaption() ?></span></td>
...
<?php echo $subject->subject_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->course_id->Visible) { // course_id ?>
	<tr id="r_course_id">
		<td class="col-sm-2"><span id="elh_subject_course_id"><?php echo $subject->course_id->FldCaption() ?></span></td>
...
<?php echo $subject->course_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->instructor_id->Visible) { // instructor_id ?>
	<tr id="r_instructor_id">
		<td class="col-sm-2"><span id="elh_subject_instructor_id"><?php echo $subject->instructor_id->FldCaption() ?></span></td>
...
<?php echo $subject->instructor_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->subject_title->Visible) { // subject_title ?>
	<tr id="r_subject_title">
		<td class="col-sm-2"><span id="elh_subject_subject_title"><?php echo $subject->subject_title->FldCaption() ?></span></td>
...
<?php echo $subject->subject_title->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->subject_description->Visible) { // subject_description ?>
	<tr id="r_subject_description">
		<td class="col-sm-2"><span id="elh_subject_subject_description"><?php echo $subject->subject_description->FldCaption() ?></span></td>
...
<?php echo $subject->subject_description->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->subject_detail->Visible) { // subject_detail ?>
	<tr id="r_subject_detail">
		<td class="col-sm-2"><span id="elh_subject_subject_detail"><?php echo $subject->subject_detail->FldCaption() ?></span></td>
		<td data-name="subject_detail"<?php echo $subject->subject_detail->CellAttributes() ?>>
<span id="el_subject_subject_detail">
<span<?php echo $subject->subject_detail->ViewAttributes() ?>>
<?php echo $subject->subject_detail->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->image->Visible) { // image ?>
	<tr id="r_image">
		<td class="col-sm-2"><span id="elh_subject_image"><?php echo $subject->image->FldCaption() ?></span></td>
		<td data-name="image"<?php echo $subject->image->CellAttributes() ?>>
<span id="el_subject_image">
<span>
<?php echo ew_GetFileViewTag($subject->image, $subject->image->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($subject->price->Visible) { // price ?>
	<tr id="r_price">
		<td class="col-sm-2"><span id="elh_subject_price"><?php echo $subject->price->FldCaption() ?></span></td>
...
<?php echo $subject->price->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->dist->Visible) { // dist ?>
	<tr id="r_dist">
		<td class="col-sm-2"><span id="elh_subject_dist"><?php echo $subject->dist->FldCaption() ?></span></td>
		<td data-name="dist"<?php echo $subject->dist->CellAttributes() ?>>
<span id="el_subject_dist">
<span<?php echo $subject->dist->ViewAttributes() ?>>
<?php echo $subject->dist->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->unit->Visible) { // unit ?>
	<tr id="r_unit">
		<td class="col-sm-2"><span id="elh_subject_unit"><?php echo $subject->unit->FldCaption() ?></span></td>
		<td data-name="unit"<?php echo $subject->unit->CellAttributes() ?>>
<span id="el_subject_unit">
<span<?php echo $subject->unit->ViewAttributes() ?>>
<?php echo $subject->unit->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->stutus->Visible) { // stutus ?>
	<tr id="r_stutus">
		<td class="col-sm-2"><span id="elh_subject_stutus"><?php echo $subject->stutus->FldCaption() ?></span></td>
...
<?php echo $subject->stutus->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($subject->create_date->Visible) { // create_date ?>
	<tr id="r_create_date">
		<td class="col-sm-2"><span id="elh_subject_create_date"><?php echo $subject->create_date->FldCaption() ?></span></td>
...
<?php echo $subject->create_date->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php } ?>
<?php
	if (in_array("lesson", explode(",", $subject->getCurrentDetailTable())) && $lesson->DetailView) {
?>
